...
def convert_xls_to_csv(project_name, s3_usage, s3_settings):
    ...
    if s3_usage:
        ...
        s3c_main.download_files_in_prefix_to_dir(project_prefix + "/input/annotations", source_dir)
    first = True
...
def run_router_rb(
    raw_pdf_folder, working_folder, output_folder, project_name, verbosity, use_docker, port, ip, s3_usage, s3_settings
):
    if use_docker:
        ...
        cmd = (
            ...
            + " rule_based_pipeline/rule_based_pipeline/main.py"
            + ' --raw_pdf_folder "'
            + raw_pdf_folder
            + '"'
            + ' --working_folder "'
...
def set_xy_ml(
    ...
    if use_docker:
        ...
        cmd = (
            config_path.PYTHON_EXECUTABLE
            + " rule_based_pipeline/rule_based_pipeline/main_find_xy.py"
            + ' --raw_pdf_folder "'
            + raw_pdf_folder
            + '"'
            + ' --working_folder "'
...
def join_output(
    ...
    for filename in os.listdir(pdf_folder):
        print(filename)
        with open(
            output_folder + r"/" + str(run_id) + r"_" + filename + r".csv", "w", encoding="UTF8", newline=""
        ) as f_out:
        ...
        if csv_name in os.listdir(output_folder):
            if s3_usage:
                ...
                s3c_main.upload_file_to_s3(
                    filepath=output_folder + r"/" + csv_name,
                    s3_prefix=project_prefix + "/output/KPI_EXTRACTION/joined_ml_rb",
                    s3_key=csv_name,
                )
            set_xy_ml(
                project_name=project_name,
                raw_pdf_folder=pdf_folder,
                working_folder=work_dir_rb,
                pdf_name=filename,
                csv_name=csv_name,
            ...
            print(
                f"File {csv_name} not in the output and hence we are not able to detect x, "
...
def main():
    ...
    parser.add_argument("--s3_usage", type=str, default=None, help="Do you want to use S3? Type either Y or N.")
    ...
    if s3_usage:
        ...
        s3c_main.download_file_from_s3(filepath=settings_path, s3_prefix=project_prefix, s3_key="settings.yaml")
    ...
    try:
        ...
        if mode in ("RB", "both"):
            ...
            end_to_end_response = end_to_end_response and run_router_rb(
                raw_pdf_folder=destination_pdf,
            ...
            if s3_usage:
                ...
                s3c_main.download_files_in_prefix_to_dir(
                    project_prefix + "/output/KPI_EXTRACTION/rb", destination_rb_infer
                )

        if mode in ("ML", "both"):
            print("Executing ML solution . . . ")
            end_to_end_response = end_to_end_response and run_router_ml(
                ext_port, infer_port, project_name, ext_ip, infer_ip
            )
            if s3_usage:
                # Download inference output
                s3c_main.download_files_in_prefix_to_dir(
                    project_prefix + "/output/KPI_EXTRACTION/ml/Text", destination_ml_infer
                )
        ...
        if end_to_end_response:
            ...
            if s3_usage:
                ...
                s3c_main.download_files_in_prefix_to_dir(project_prefix + "/input/pdfs/inference", destination_pdf)
            ...
            if project_settings["extraction"]["store_extractions"]:
                ...
                if s3_usage:
                    s3c_interim.download_files_in_prefix_to_dir(
                        project_prefix + "/interim/ml/extraction", source_extraction_data
                    )
                    s3c_main.upload_files_in_dir_to_prefix(
                        source_extraction_data, project_prefix + "/output/TEXT_EXTRACTION"
                    )
